...
$from->diff($to)->y;
    ...
    try {
        ...
        /*
        $listadeEvolucao = Exame::getListaEvolucao($dbevolucao->paciente);
        $evolucaoResult = Exame::getUnicEvolucao($dbevolucao->id);
        ...
        */
        ...
        header("Location: Evolucao2.php?paciente2=$dbevolucao->paciente&id=$dbevolucao->id");
...
if(!empty($listadeEvolucao) && !isset($_GET['novo']) ) {
...
} else if(!empty($listadeEvolucao) && isset($_GET['novo']) ) {
    
    loadTemplateView3('Evolucao2new','leftEvolucao',$user+
    ['today'=>$today,
    'exception'=>$exception,
    'listadeEvolucao'=>$listadeEvolucao,
    'evolucaoResult'=>$evolucaoResult,
