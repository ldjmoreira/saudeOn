...
class Agenda extends Model {
    ...
    public static function getListaSinaisVitais($args){
        ...
        programacao_sinais_vitais_detalhes.ciente,
        programacao_sinais_vitais_detalhes.cod_sinal_vital,
        programacao_sinais_vitais_detalhes.registro_ativo,
        sinal_vitals.descricao
        FROM programacao_sinais_vitais_detalhes
        INNER JOIN sinal_vitals ON sinal_vitals.id = programacao_sinais_vitais_detalhes.cod_sinal_vital
        WHERE programacao_sinais_vitais_detalhes.paciente= $args 
        AND programacao_sinais_vitais_detalhes.intervalo != 0 
        ORDER BY programacao_sinais_vitais_detalhes.id DESC
        ";
        $result = Database::getResultFromQuery($sql);
    ...
    public static function getListaProgMedicamentos($args){
        $objects=[];
        $sql = "SELECT programacao_medicamentos_2.id,
        programacao_medicamentos_2.medicamento_id,
        programacao_medicamentos_2.via,
        programacao_medicamentos_2.dose,
        programacao_medicamentos_2.intervalo,
        programacao_medicamentos_2.datahora,
        programacao_medicamentos_2.aprazada,
        programacao_medicamentos_2.ativo,
        programacao_medicamentos_2.duracao,
        programacao_medicamentos_2.observacao,
        programacao_medicamentos_2.ciente
         FROM programacao_medicamentos_2
         WHERE programacao_medicamentos_2.id= $args 
         ORDER BY programacao_medicamentos_2.id DESC";
    ...
    public static function getListaProgCuidados($args){
        ...
        programacao_procedimentos_detalhadas.cuidador_id,
    ...
    public static function selectDouble2($tableName1,$column1,$column2,$condition,$resp) {
        $objects = [];
        $sql = "       SELECT $column1 , $column2
        FROM $tableName1 WHERE $condition = $resp ";
    ...
    public static function listaProfissionais($arg,$arg2){
        $objects = [];
        $sql = "SELECT 
         profissionals.nome_Prof,
         cbo.descricao, 
         ...
         agenda_profissional_detalhadas.duracao, 
         agenda_profissional_detalhadas.periodicidade, 
         ...
         WHERE agenda_profissional_detalhadas.profissional= $arg 
         AND agenda_profissional_detalhadas.softdelete != 1
         AND agenda_profissional_detalhadas.ativo = 1
         AND (agenda_profissional_detalhadas.data >= '$arg2' OR agenda_profissional_detalhadas.duracao != 0 )
    ...
    public static function listaProfissionaisOld($arg){
        $objects = [];
        $sql = "SELECT 
        profissionals.nome_Prof,
        pacientes.name, 
    ...
    public function updateSTR($tableName1,$columns) {
        ...
        if($tableName =="programacao_medicamentos_2"){
            $this->validate_ProgMed();

            if(!$this->aprazada) $this->aprazada = 0;
            if(!$this->operador) $this->operador = null;
            if(!$this->id) $this->id = null;
            if(!$this->ciente) $this->ciente = null;
        }
    ...
    public static function getListaProfPacAll($args){
        $objects = [];
        $sql = " SELECT 
        profissionals.nome_Prof as profissional, 
        cbo.descricao, 
        agenda_profissional_detalhadas.motivo,
        agenda_profissional_detalhadas.id, 
        agenda_profissional_detalhadas.data, 
    ...
    public static function getListaProfPac($args,$arg2){ 
        $objects = [];
        $sql = "SELECT 
        profissionals.nome_Prof,
         cbo.descricao, 
         ...
         INNER JOIN agenda_profissional_detalhadas ON agenda_profissional_detalhadas.profissional = profissionals.profissional_id 
         INNER JOIN cbo ON cbo.id = profissionals.CBO 
         WHERE agenda_profissional_detalhadas.paciente= $args 
         AND agenda_profissional_detalhadas.softdelete != 1
         AND agenda_profissional_detalhadas.ativo = 1
         AND (agenda_profissional_detalhadas.data >= '$arg2' OR agenda_profissional_detalhadas.duracao != 0 )
         ORDER BY agenda_profissional_detalhadas.data DESC
    ...
    public static function getListaCuidados($args ){
        ...
        FROM programacao_procedimentos_detalhadas
         INNER JOIN procedimentos ON programacao_procedimentos_detalhadas.procedimento = procedimentos.id
          ...
          WHERE programacao_procedimentos_detalhadas.paciente= $args 
          AND programacao_procedimentos_detalhadas.softdelete != 1
         AND programacao_procedimentos_detalhadas.ativo = 1
          ORDER BY programacao_procedimentos_detalhadas.id DESC
          ";
    ...
    public static function getListaMedicamentos($args){
        ...
        $objects = [];
        $sql=" SELECT medicamentos.name,
        medicamentos.apresentacao,
        programacao_medicamentos_2.dose,
        programacao_medicamentos_2.id,
        programacao_medicamentos_2.via,
        programacao_medicamentos_2.intervalo,
        programacao_medicamentos_2.datahora,
        programacao_medicamentos_2.aprazada,
        programacao_medicamentos_2.ciente
        FROM programacao_medicamentos_2
        INNER JOIN medicamentos ON programacao_medicamentos_2.medicamento_id = medicamentos.id
        WHERE programacao_medicamentos_2.paciente= $args 
        AND programacao_medicamentos_2.softdelete != 1
        AND programacao_medicamentos_2.ativo = 1
        ORDER BY programacao_medicamentos_2.id DESC
        ";
        $result = Database::getResultFromQuery($sql);
    ...
    public static function getListaMedicamentosAll($args){
        $objects = [];
        ...
        WHERE programacao_medicamentos_detalhadas.paciente= $args 
    ...
    private function validate_ProfPac(){
        ...
        if(!$this->turno) {
            $errors['turno'] = 'Turno é um campo abrigatório.';
        }
        if(!$this->data) {
        ...
        if(!$this->motivo) {
            $errors['motivo'] = 'Motivo é um campo abrigatório.';
        }
        if($this->periodicidade == '') {
            $errors['periodicidade'] = 'Intervalo é um campo abrigatório.';
        }
    ...
    private function validate_ProgMed(){
        $errors = [];
        if(!$this->medicamento_id) {
            $errors['medicamento_id'] = 'Medicamento é um campo abrigatório.';
        }
        if(!$this->via) {
            $errors['via'] = 'Via é um campo abrigatório.';
        }
        if(!$this->dose) {
            $errors['dose'] = 'Dose é um campo abrigatório.';
        }
